...
@interface DGCSVGAPlayer ()

@property (nonatomic, strong) CALayer *drawLayer;
@property (nonatomic, strong) NSArray<DGCSVGAAudioLayer *> *audioLayers;
...
@property (nonatomic, copy) NSDictionary<NSString *, DGCSVGAPlayerDynamicDrawingBlock> *dynamicDrawings;
...
//    int fps = MIN(10, self.videoItem.FPS);
//    self.displayLink.frameInterval = 60 / fps;
...
- (void)clearAudios {
    for (DGCSVGAAudioLayer *layer in self.audioLayers) {
...
- (void)draw {
    ...
    [self.videoItem.audios enumerateObjectsUsingBlock:^(DGCSVGAAudioEntity * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        DGCSVGAAudioLayer *audioLayer = [[DGCSVGAAudioLayer alloc] initWithAudioItem:obj videoItem:self.videoItem];
...
- (void)update {
    ...
    for (DGCSVGAContentLayer *layer in self.contentLayers) {
...
- (void)setVideoItem:(DGCSVGAVideoEntity *)videoItem {
...
- (void)setImage:(UIImage *)image forKey:(NSString *)aKey {
    ...
    if (self.contentLayers.count > 0) {
        for (DGCSVGAContentLayer *layer in self.contentLayers) {
            if ([layer isKindOfClass:[DGCSVGAContentLayer class]] && [layer.imageKey isEqualToString:aKey]) {
...
- (void)setDrawingBlock:(DGCSVGAPlayerDynamicDrawingBlock)drawingBlock forKey:(NSString *)aKey {
...
- (NSDictionary<NSString *,DGCSVGAPlayerDynamicDrawingBlock> *)dynamicDrawings {
