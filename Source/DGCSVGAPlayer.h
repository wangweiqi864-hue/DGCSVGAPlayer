...
@class DGCSVGAVideoEntity, DGCSVGAPlayer;

@protocol DGCSVGAPlayerDelegate <NSObject>

@optional
- (void)svgaPlayerDidFinishedAnimation:(DGCSVGAPlayer *)player ;

- (void)svgaPlayer:(DGCSVGAPlayer *)player didAnimatedToFrame:(NSInteger)frame;
- (void)svgaPlayer:(DGCSVGAPlayer *)player didAnimatedToPercentage:(CGFloat)percentage;
...
typedef void(^DGCSVGAPlayerDynamicDrawingBlock)(CALayer *contentLayer, NSInteger frameIndex);

@interface DGCSVGAPlayer : UIView

@property (nonatomic, weak) id<DGCSVGAPlayerDelegate> delegate;
